    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-opacity-60 backdrop-blur-xs  ">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-2xl relative transform scale-95 animate-slide-up border border-blue-200  ">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-gray-600 text-base mb-1">Role: <span className="font-semibold capitalize">{user.role}</span></p> {/* ব্যবহারকারীর ভূমিকা */}
          <p className="text-gray-500 text-sm">ID: {user.id}</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-sm">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Owned Projects ({user.ownedProjects?.length || 0})</h3>
          ...
          <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-sm">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Memberships ({user.memberships?.length || 0})</h3>
            {user.memberships && user.memberships.length > 0 ? (
              <ul className="space-y-2 max-h-40 overflow-y-auto pr-2">
                {user.memberships.map((membership) => (
                  <li key={membership.project_id}>{membership.project_name} (<span className="capitalize">{membership.role}</span>)</li>
                ))}
              </ul>
            ) : (
              <p className="text-gray-600">No memberships.</p>
            )}
          </div>
        </div>
